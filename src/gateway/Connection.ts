...
let parse = (func: { (payload: any): void; (payload: any): void; (payload: any): void; (arg0: any): void; }) => {
	return (data: string) => {
...
export class Connection {
	...
	connect(): Promise<{ timeReady: number, socket: Connection }> {
		...
		return new Promise((resolve, reject) => {
			...
			this.socket?.once('open', () => {
				...
				this.socket?.once('message', parse((payload: { d: { heartbeat_interval: number; }; }) => {
	...
	identify(): Promise<{ timeReady: number, socket: Connection }> {
		return new Promise((resolve, reject) => {
			...
			this.socket?.on('message', parse((payload: { s: number; op: number; t: string; d: any; }) => {
				this.s = payload.s;
			...
			this.socket?.once('message', parse((payload: { t: string; d: { session_id: number; }; op: number; }) => {
				if (payload.t === 'READY') {
